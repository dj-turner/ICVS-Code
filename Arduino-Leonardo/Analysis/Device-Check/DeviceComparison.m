...
newDataDates = ["26-Mar-2024", "27-Mar-2024"];
newData = newData(startsWith(string(newData.DateTime), newDataDates),:);
...
newRG = NaN(1,length(newDevices)*length(newDataDates));
for device = 1:length(newDevices)
    for date = 1:length(newDataDates)
        idx = strcmp(newData.Device, newDevices(device))... 
              & startsWith(string(newData.DateTime), newDataDates(date))...
              & newData.InputValue == 255;
        ...
        newRG(device+(date-1)*length(newDevices)) = redValue / greenValue;
    end
end
...
set(gca, 'xticklabel', ["G1", "G2", "G3", "G4", "G5", "G6", "G7", "G8", "Mitch1", "Test1", "Josh1", "Mitch2", "Test2", "Josh2"]);
...
for led = 1:length(colours)
    ...
    for device = 1:length(newDevices)
        for date = 1:length(newDataDates)
            nexttile(led)
            idx = strcmp(newData.LED, colours(led))...
                  & strcmp(newData.Device, newDevices(device))...
                  & startsWith(string(newData.DateTime), newDataDates(date));
